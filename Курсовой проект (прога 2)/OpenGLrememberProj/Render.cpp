...
ObjFile waterObj, boatObj, ballObj, TowerObj;
Texture waterTex, boatTex, TowerTex;

BoatСontrolSystem Boat = BoatСontrolSystem();
...
void keyUpEvent(OpenGL *ogl, int key)
{
	//движение вперед
	if (key == 'W') {
		Boat.FlagAcceleration = false;
	}

	//движение назад
	if (key == 'S') {
		Boat.FlagDeceleration = false;
	}
}
...
void initRender(OpenGL *ogl)
{
	...
	loadModel("models\\water.obj_m", &waterObj);
	
	waterTex.loadTextureFromFile("textures//water.bmp");
	waterTex.bindTexture();

	loadModel("models\\boat.obj_m", &boatObj);
	boatTex.loadTextureFromFile("textures//boat.bmp");
	boatTex.bindTexture();

	loadModel("models\\ball.obj_m", &ballObj);
...
void RenderGUI(OpenGL *ogl)
{
	...
	std::stringstream ss;
	ss << "Всего фонариков в полете - " << Max_Ball << std::endl;
	ss << "Количество пойманных фонариков - " << Numbar_ball << std::endl;
	ss << "Радиус лова - " << Rboat << std::endl;
	ss << "Осталось времени: " << NextTime << std::endl;
